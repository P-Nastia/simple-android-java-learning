...
server_up() {
    echo "Server up..."
    docker pull nastiapyv/task-api:latest
    docker stop task-api_container || true
    docker rm task-api_container || true
    docker run -d --restart=always -v /volumes/pizushi-asp/images:/app/images --name task-api_container -p 4179:8080 nastiapyv/task-api
}
